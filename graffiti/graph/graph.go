...
func (g *Graph) GetNode(ctx context.Context, i Identifier) *Node {
	ctx, span := tracer.Start(ctx, "Graph.GetNode", trace.WithAttributes(attribute.Key("NodeID").String(string(i))))
	defer span.End()
...
// GetNodeAll from Identifier, all revisions, maybe filtered by g.Context.TimeSlice
...
func (g *Graph) MarshalJSON() ([]byte, error) {
	// We cannot get the otel ctx as we have to satisty the json.Marshaler interface
	ctx, span := tracer.Start(context.Background(), "Graph.MarshalJSON")
	defer span.End()
